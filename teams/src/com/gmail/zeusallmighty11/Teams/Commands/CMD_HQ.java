...
public class CMD_HQ
{
    ...
    public static void execute(Player sender, String[] args)
    {
        ...
        if (tp.getTeam().getHQ() == null)
        {
            sender.sendMessage("§8[§3Teams§8] §cThere is no valid HQ location.");
            return;
        }
        ...
        for (Entity e : sender.getNearbyEntities(30.0, 30.0, 30.0))
        {
            ...
            {
                tp.init_safeTeleport(tp.getTeam().getHQ());
        ...
        if (!wait)
        {
            tp.getAsPlayer().teleport(tp.getTeam().getHQ());
            sender.sendMessage("§8[§3Teams§8] §aTeleported to hq location.");
        }
    }
}
